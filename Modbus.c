...
struct Modbus_Config_s Modbus_Config;

#define CHECK_BIT(var,pos) !!((var) & (1 << (pos)))

unsigned short holdingReg[100];
...
void Transmit(unsigned char * data, unsigned char length) {
  ...
  TX_LED_Write(0xFF);
  for(i=0;i!=length;i++){
     Modbus_Config.Uart_PutByte(data[i]);
  }
  
  //CyDelay(length);
  responseFinished(length);
  TX_LED_Write(0x00);
...
{
    RX_LED_Write(0x00);
    if(received[0] == Modbus_Config.SlaveAddress[0]){
        if(checkCRC() == 1){        
		if(received[1] == 0x01){
            readCoil();
        }
...
/******************************************************************************/
  ...
  response[0] = Modbus_Config.SlaveAddress[0];
  response[1] = 0x01;
...
//  for(i=0;i!=j;i++){
//     Modbus_Config.Uart_PutByte(response[i]);
//  }
//  responseFinished(9);
  Transmit(response, (j));
  i=0;  
...
/******************************************************************************/
  ...
  // prevent errors whilst programming new address settings. 
  uint8_t Slave_Address = Modbus_Config.SlaveAddress[0];
