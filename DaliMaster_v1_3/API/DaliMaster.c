...
void `$INSTANCE_NAME`_Direct_Pulse() {
	...
	if ( rx_state == FREE ) {
        ...
        bitPtr = 0;
        bit = 1;
        previous = Dali_Timer_ReadCounter();
...
//        }
        ...
        int now = Dali_Timer_ReadCounter();
        diff  = previous - now;
        ...
        if ( (diff > 300) && (diff <= 550) ) {
            ...
            Dali_result |= Dali_BACKFRAME; // Has Backframe
        ...
        if ( bitPtr >= 32 ) {
            ...
            ARCVal = 0x00;
            for ( i = 0 ; i < 8 ; i++ ) {
                rxAddr |= ( (frame[i]&0x01) << (8-i) );
            }
            for ( i = 9 ; i < 18 ; i++ ) {
                ARCVal |= ( (frame[i]& 0x01) << (8-(i-8)) );
            }
            
            Dali_Control_Reg_1_Write(0xFF);
        	CyDelayUs(1);
        	Dali_Control_Reg_1_Write(0x00);
        	CyDelayUs(1);
            Dali_Timer_Stop();
        	Dali_Timer_WriteCounter(0xFFFF);
            
            Dali_result = (rxAddr<<16);
            Dali_result |= (ARCVal<<8);
...
void `$INSTANCE_NAME`_Dali_Listen_Mode() {
    `$INSTANCE_NAME`_Dali_Out_SetDriveMode(`$INSTANCE_NAME`_Dali_Out_DM_DIG_HIZ);
    `$INSTANCE_NAME`_Dali_Out_Write(0x00); // bus high!
    `$INSTANCE_NAME`_Dali_In_SetDriveMode(`$INSTANCE_NAME`_Dali_In_DM_DIG_HIZ);
    `$INSTANCE_NAME`_Start();
}
